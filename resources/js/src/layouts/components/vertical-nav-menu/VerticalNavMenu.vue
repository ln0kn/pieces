...
<template>
    <div class="parentx">
        ...
        <vs-sidebar class="v-nav-menu items-no-padding" v-model="isVerticalNavMenuActive" ref="verticalNavMenu" default-index="-1" :click-not-close="clickNotClose" :reduce-not-rebound="reduceNotRebound" :parent="parent" :hiddenBackground="clickNotClose" :reduce="reduce" v-hammer:swipe.left="onSwipeLeft">
            ...
            <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
                ...
                <div class="header-sidebar flex items-end justify-between" slot="header">
                    ...
                    <router-link tag="div" class="vx-logo cursor-pointer flex items-center" to="/">
                        ...
                        <span class="vx-logo-text text-primary" v-show="isMouseEnter || !reduce" v-if="title">BOUDACOM</span>
                    </router-link>
                    <!-- /Logo -->
                    ...
                    <div>
                        ...
                        <template v-else-if="!showCloseButton && !verticalNavMenuItemsMin">
                            <feather-icon id="btnVNavMenuMinToggler" class="mr-0 cursor-pointer" :icon="reduce ? 'CircleIcon' : 'DiscIcon'" svg-classes="stroke-current text-primary" @click="toggleReduce(!reduce)" />
                        </template>
                    </div>
                    <!-- /Menu Toggle Buttons -->
                </div>
                ...
                <VuePerfectScrollbar ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" :key="$vs.rtl">
                    <template v-for="(item, index) in menuItemsUpdated">
                        ...
                        <template v-else-if="!item.header">
                            ...
                            <!-- Nav-Item -->
                            <v-nav-menu-item v-if="!item.submenu" :key="`item-${index}`" :index="index" :to="item.slug !== 'external' ? item.url : null" :href="item.slug === 'external' ? item.url : null" :icon="item.icon" :target="item.target" :isDisabled="item.isDisabled" :slug="item.slug">
        ...
        <vs-sidebar click-not-close position-right class="v-nav-menu items-no-padding" v-model="isSidebarActiveLocal" ref="verticalNavMenu" :hiddenBackground="clickNotClose">
            <div class="mt-6 flex items-center justify-between px-6">
                <h4>STOCK</h4>
                <feather-icon icon="XIcon" @click="$store.commit('SIDEBAR_STATE', false)" v-model="$store.sideBarState" class="cursor-pointer"></feather-icon>
            </div>
            <div class="mt-6 flex items-center justify-between px-6">
                <small class="whitespace-no-wrap"><h4>vente du jour : {{$store.state.mtnVte}}</h4> </small>
            </div>
            <div class="mt-6 flex items-center justify-between px-6">
                <small class="whitespace-no-wrap"><h4>encaissée {{$store.state.mtnVersee}} </h4></small>
            </div>
            <vs-divider class="mb-0"></vs-divider>
            <vs-input size="small" icon-pack="feather" icon="icon-search" v-model="stockSearch" class="ml-10 is-label-placeholder" />


            <VuePerfectScrollbar ref="mainSidebarPs" class="scroll-area--nofications-dropdown p-0 mb-10">
                <ul class="bordered-items">
                    <li v-for="ntf in listeStock" class="flex justify-between px-4 py-4 notification cursor-pointer">
                        <div class="flex items-start">

                            <div>
                                <span class="font-medium block notification-title " :class="[`text-primary`]">{{ ntf[1] }}</span>
                                <small class="whitespace-no-wrap">Boutique:{{ ntf[0]['BOUTIQUE'] }} - </small>
                                <small>Rakieta:{{ ntf[0]['RAKIETA'] }} - </small>
                                <small>Coram:{{ ntf[0]['CORAM'] }}</small>
                            </div>
                        </div>
                    </li>
                </ul>
        ...
        watch: {
            ...
            stockSearch() {
                ...
                if (this.stockSearch.length > 0) {
                    let obj = [];
                    let value = this.stockSearch.toLowerCase();
